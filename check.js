...
function CheckValid(str , obj){
 ...
 function sortObjbasedonValue(inputObject){
  ...
  keyValueArray.sort((a, b) => {
    ...
    // If values are the same, sort by alphabetic character order
    return a[0].localeCompare(b[0]);
  });
  
  // Convert sorted array back to an object
  const sortedObject = Object.fromEntries(keyValueArray);
  // convert in string for length 5
   const stringFiveComman = Object.keys(sortedObject)?.slice(0,5)?.join("")

  return stringFiveComman
 }
...
function getResult(file){
  ...
  for(let i =0; i < file.length ; i++){
    let characterMap ={}
   const string = file[i]?.split("-") // make array seprated by -
  const lastData = string?.pop()  // remove last value
  const number = lastData?.split("[")[0] // getting Number
  const stringForCheck = lastData?.split("[")?.[1]?.replace("]" , "") // for string with five mostcharacter


  for(let i =0 ; i < string.length ; i++){
     const newobj = CheckValid(string[i] ,characterMap )
    characterMap ={...newobj}
  }
   
  const stringWithFiveComman = sortObjbasedonValue(characterMap)  // string by input
  // console.log("help",characterMap , stringForCheck , stringWithFiveComman)
  const isValid = stringForCheck == stringWithFiveComman ? true : false
 ...
 // my test case 
const arr = ["aaaaa-bbb-z-y-x-123[abxyz]" , "a-b-c-d-e-f-g-h-987[abcde]" , "totally-real-nacho-sequence-200[decoy]"]
...
console.log(newStr?.length , newStr[newStr?.length-1] )
console.log(getResult(newStr))
